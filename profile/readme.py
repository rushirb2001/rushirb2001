...
def projects(base, key):
    ...
    if s.get("layout", "cards") == "cards":
        cards = [picture(base, "repo", CARD_W, r["repo"], f'https://github.com/{login}/{r["repo"]}',
                         username=login, repo=r["repo"], status=r.get("status"),
                         description=r.get("description"), install=r.get("install"))
                 for r in repos]
        return block(header + "<br/>", rows(cards, 2))
    params = {
...
def readme(base):
    ...
    out.append(block(picture(base, "hero", 880, h["name"], name=h["name"], eyebrow=h["eyebrow"],
                             line=h["line"], tagline="|".join(h["principles"]))))
